...
def fix():
    ...
    a_student1 = User.query.filter(db.and_(User.username==wrong)).first()
    a_student2 = Pickedcourses.query.filter(db.and_(Pickedcourses.student_id==wrong)).first()
    a_student3 = Receiptlog.query.filter(db.and_(Receiptlog.student_id==wrong)).first()
    a_student4 = Registration.query.filter(db.and_(Registration.student_id==wrong)).first()
    a_student5 = Student.query.filter(db.and_(Student.student_id==wrong)).first()
    a_student6 = Wallet.query.filter(db.and_(Wallet.student_id==wrong)).first()
